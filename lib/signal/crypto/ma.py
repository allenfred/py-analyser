...
def is_dead_cross(index, ma1, ma2):
    """
    MA/EMA 死亡交叉（5/20）/（10/20）/ （10/30）
    ...
    if ma_down() and ma1[index] < ma2[index] and ma1[index - 1] > ma2[index - 1]:
        return True
...
def has_support_patterns(index, df):
    ...
    if df.iloc[index]['swallow_up'] > 0 or df.iloc[index]['down_rise'] > 0 \
            ...
            or df.iloc[index]['CDLTAKURI'] > 0 or df.iloc[index]['CDLPIERCING'] > 0:
        return True
