...
# 定义 candle 对象:
class HKDailyCandle(Base):
    # 表的名字:
    __tablename__ = 'hk_daily_candles'
...
class HKDailyCandleDao:
    def __init__(self):
        self.session = DBSession()

    def find_latest_candle(self):
        s = text("select trade_date, open, close, high, low from hk_daily_candles order by trade_date desc limit 1;")
    ...
    def find_by_trade_date(self, trade_date):
        s = text("select ts_code, open, close, high, low from hk_daily_candles where trade_date = :trade_date;")
    ...
    def add_one(self, candle):
        ...
        rows = self.session.query(HKDailyCandle.id).filter(HKDailyCandle.ts_code == candle['ts_code']).filter(
            HKDailyCandle.trade_date == candle['trade_date']).first()
    ...
    def bulk_insert(self, df):
        ...
        try:
            self.session.bulk_insert_mappings(HKDailyCandle, items)
            self.session.commit()
    ...
    def reinsert(self, df):
        ...
        try:
            self.session.execute("delete from hk_daily_candles where ts_code = :ts_code", {"ts_code": ts_code})
            self.session.bulk_insert_mappings(HKDailyCandle, items)
            self.session.commit()
        except Exception as e:
            print('Error:', e)
    ...
    def bulk_upsert(self, df):
        ...
        for index, candle in df.iterrows():
            ...
            try:
                row = self.session.query(HKDailyCandle).filter(HKDailyCandle.ts_code == candle['ts_code']).filter(
                    HKDailyCandle.trade_date == candle['trade_date']).first()
