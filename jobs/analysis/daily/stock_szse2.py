...
if __name__ == "__main__":
    ...
    cnt_stmts = stockDao.session.execute(text("select count(ts_code) as count from stocks where ("
                                              "scan_date is null"
                                              " or scan_date < :scan_date) and exchange = 'SZSE' limit "
                                              + str(limit)).params(scan_date=scan_date))

    cnt_result = cnt_stmts.fetchall()
